...
class Gradient(QWidget):
    ...
    def _find_stop_handle_for_event(self, e, to_exclude=None):
        ...
        if e.position().y() >= midpoint - self._handle_h and e.position().y() <= midpoint + self._handle_h:
            for n, (stop, color) in enumerate(self._gradient):
                if to_exclude and n in to_exclude:
                    ...
                    continue
                if e.position().x() >= stop * width - self._handle_w and e.position().x() <= stop * width + self._handle_w:
                    return n
