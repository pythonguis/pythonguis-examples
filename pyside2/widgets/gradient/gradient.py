from PySide2.QtCore import QRect, QRectF, QSize, Qt, Signal
from PySide2.QtGui import QColor, QLinearGradient, QPainter, QPen
from PySide2.QtWidgets import QColorDialog, QSizePolicy, QWidget
        ...
        if e.y() >= midpoint - self._handle_h and e.y() <= midpoint + self._handle_h:
            for n, (stop, color) in enumerate(self._gradient):
                if to_exclude and n in to_exclude:
                    ...
                    continue
                if e.x() >= stop * width - self._handle_w and e.x() <= stop * width + self._handle_w:
                    return n
